...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><strong>地址:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>"/* +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>"*/;
    } else {
        var domain = up.getOption('domain');
        url = "http://image.mujinnong.com/" + encodeURI(res.key);
        str = "<div><strong>地址:</strong><a href=" + url + " target='_blank' > " + url + "</a></div>"/* +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>"*/;
    }
